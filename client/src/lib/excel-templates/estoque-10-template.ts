...
export class Estoque10Template implements ExcelTemplate {
  ...
  private calculateResumoGeral(totaisChaoCheio: number[], totaisChaoVazio: number[], totaisGarrafeiraVazia: number[]) {
    return {
      ...
      // GARRAFAS (apenas Chão Cheio + Chão Vazio) - Garrafeiras Vazias NÃO entram
      totalGarrafas300: ((totaisChaoCheio[1] || 0) + (totaisChaoVazio[1] || 0)) * 24,
      totalGarrafas600: ((totaisChaoCheio[3] || 0) + (totaisChaoVazio[3] || 0)) * 24,
      totalGarrafas1L: ((totaisChaoCheio[5] || 0) + (totaisChaoVazio[5] || 0)) * 12,
